...
-   Step 3: Update Allele count, Allele frequency and site depth.
-   Step 4: Calculation of Expected heterozygosity by site, Effective cardinality by site, Proportion of polyclonal samples (heterozygous loci) by site.
-   Step 5: Second filter of sites whose alternative alleles only appear on polyclonal samples and/or sites where more than 50% (this value can be changed) of the samples are polyclonal.
-   Step 6: Calculation of nucleotide diversity, FST, TsTv ratio, and Tajima by interval.
-   Step 7: Population structure. Generation of the haplotype matrix, the euclidean distance matrix, and the eigenvectors and eigenvalues.
...
load('ColombiaGates_1st_2nd_filters.RData')
rm(list = ls()[-grep('rGenome_object|vcf_object',ls())])

rGenome_object@data$metadata = data.frame(sample = names(vcf_object)[-1:-9])

save.image('ColombiaGates_1st_2nd_filters.RData')
...
```{r}
rGenome_object@data$metadata %<>% mutate(Population = case_when(
    grepl('Colombia_301010', sample) ~ 'Buenaventura_2016',
    grepl('Colombia_301021', sample) ~ 'Tierralta_2016',
    grepl('Colombia_301111', sample) ~ 'Choco_2016',
    grepl('Colombia_301031', sample) ~ 'Tumaco_2016',
    grepl('SP0101254',sample) ~ 'Buenaventura_2020',
    grepl('SP0112286',sample) ~ 'Quibdo_2020',
    grepl('SP0126288',sample) ~ 'Guapi_2020'
  ))
```
...
```{r}
rGenome_object@data$metadata %>% ggplot(aes(x = SampleAmplRate, fill = Population))+
...
rGenome_object = filter_samples(obj = rGenome_object, v =
                rGenome_object@data$metadata$SampleAmplRate >= .75)
```
...
```{r}
rGenome_object@data$loci_table = cbind(rGenome_object@data$loci_table,
                                       get_AC(obj = rGenome_object))
rGenome_object = filter_loci(rGenome_object,
                             v = rGenome_object@data$loci_table$Cardinality > 1)
```
...
rGenome_object = filter_loci(rGenome_object, v = rGenome_object@data$loci_table$LocusAmplRate >= .75)
```

6. Update alternative alleles

```{r}
rGenome_object@data$loci_table$ALT =
  ifelse(rGenome_object@data$loci_table$REF !=
           gsub(',([ATCG]|\\*)+',
                '',
                gsub(':\\d+',
                     '',
                     rGenome_object@data$loci_table$Alleles)),
         gsub(':\\d+',
              '',
              rGenome_object@data$loci_table$Alleles),
         gsub('^([ATCG]|\\*)+,',
              '',
              gsub(':\\d+', '', rGenome_object@data$loci_table$Alleles))
       )
```
...
```{r}
rGenome_object@data$loci_table$TypeOf_Markers =
...
```{r}
rGenome_object@data$loci_table$ObsHet = get_ObsHet(rGenome_object, by = 'loci', w = 1, n = 1)
```

9. Calculate fraction of Heterozygous samples per Alternative alleles per site

```{r}
rGenome_object@data$loci_table$frac_ofHet_pAlts =
...
```{r}
rGenome_object@data$loci_table %>% ggplot(aes(x = frac_ofHet_pAlts))+
...
```{r}
rGenome_object@data$loci_table %<>% mutate(ALT_FILTER =case_when(
...
```{r}
rGenome_object@data$loci_table %>%
    ggplot(aes(x = ObsHet,
...
```{r}
rGenome_object = filter_loci(rGenome_object,
            v = !(rGenome_object@data$loci_table$TypeOf_Markers %in%
...
```{r}
rGenome_object@data$loci_table = mean_ObsHet(rGenome_object, gff = 'genes.gff')
```

14. Define a threshold for trusted variants
```{r}
ObsHet_Threshold = quantile(rGenome_object@data$loci_table %>%
...
```{r}
rGenome_object@data$loci_table %<>% mutate(ObsHet_Filter = case_when(
    mean_ObsHet > ObsHet_Threshold ~ 'OUT',
    mean_ObsHet <= ObsHet_Threshold ~ 'PASS'
  ))
```

17. Visualize which genomic regons PASS the filter
```{r}
rGenome_object@data$loci_table %>%
    mutate(TypeOf_Markers2 = case_when(
      TypeOf_Markers == 'SNP' ~ 'SNP',
...
```{r}
rGenome_object@data$loci_table = SNP_density(rGenome_object, gff = 'genes.gff')

SNP_density_threshold = quantile(rGenome_object@data$loci_table %>%
...
23. Visualize which genomic regons PASS the filter
```{r}
rGenome_object@data$loci_table %>%
    mutate(TypeOf_Markers2 = case_when(
      TypeOf_Markers == 'SNP' ~ 'SNP',
...
rGenome_object = 
  filter_loci(rGenome_object,
              v = rGenome_object@data$loci_table$Alignment_Filter == 'PASS')
```
...
```{r}
rGenome_object@data$metadata$fracHet = 
...
30. Save data
```{r}
rm(list = ls()[-grep('rGenome_object',ls())])
save.image('ColombiaGates_filtered_rGenome.RData')
```
